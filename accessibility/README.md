...
2. Find the CH343's COM port (Device Manager → Ports). Set `PORT` at the top of
   `makcu_access.py`.
...
3. In `makcu_access.py`, map those keys in `PEDAL_MAP`:

   ```python
   PEDAL_MAP = {
       "f13": "RT",   # left pedal  -> right trigger (fire), latched
       "f14": "A",    # middle pedal-> A, latched
       "f15": "LT",   # right pedal -> left trigger (ADS), latched
   }
   ```
...
> **This needs the modified firmware built and flashed to the Left MCU.** The
> new code is in `extracted/.../km_inject.c`. It has NOT been tested on hardware
> here — verify behavior after flashing.
...
matter too, extending the filter to it is a follow-up (same technique, a few
more lines in each `apply_*`). Say the word.
