...
// CFG_TUSB_DEBUG_PRINTF points here for the temporary level-2 enumeration
// trace. TinyUSB often builds a line with several printf calls, so collect
// fragments and add one stable prefix before emitting it on UART0.
int pass_tusb_debug_printf(const char *fmt, ...) {
    static char   line[256];
    static size_t used = 0;
    char          part[160];
    va_list       ap;
    va_start(ap, fmt);
    int n = vsnprintf(part, sizeof(part), fmt, ap);
    va_end(ap);
    if (n <= 0) return n;
    ...
    for (size_t i = 0; i < take; ++i) {
        ...
        if (c == '\n' || used == sizeof(line) - 1) {
            char out[280];
            int out_n = snprintf(out, sizeof(out), "[L][USB] %.*s\n",
                                 (int)used, line);
            if (out_n > 0) km_uart_write(out, (size_t)out_n);
            used = 0;
            if (c != '\n') line[used++] = c;
        }
    }
...
static void notify_target_state(uint8_t frame_type, const char *state) {
    bool sent = ipc_send(frame_type, 0, 0, NULL, 0);
    char msg[96];
...
void tud_mount_cb(void) {
    L_LOG("L tud_mount_cb");
    notify_target_state(FRAME_TARGET_MOUNTED, "MOUNTED");
}

void tud_umount_cb(void) {
    L_LOG("L tud_umount_cb");
    notify_target_state(FRAME_TARGET_RESET, "RESET source=umount");
}
...
#ifndef CTL_PENDING_TIMEOUT_MS
#define CTL_PENDING_TIMEOUT_MS 5000
#endif
...
// SETUP callbacks, IPC RX, and the periodic watchdog run on different tasks.
// Keep the small control-state snapshots atomic; never call USB/UART APIs
// while holding this lock.
static portMUX_TYPE           ctl_lock        = portMUX_INITIALIZER_UNLOCKED;
...
static void pass_driver_init(void) {
    ...
    open_ep_count = 0;
    pass_mounted  = false;
    portENTER_CRITICAL(&ctl_lock);
    ctl_pending  = false;
    ctl_setup_us = 0;
    portEXIT_CRITICAL(&ctl_lock);
    synth_timer_start_once();
}

static void pass_driver_reset(uint8_t rhport) {
    L_LOG("L pass_driver_reset rhport=%u", rhport);
    notify_target_state(FRAME_TARGET_RESET, "RESET source=bus_reset");
    (void)rhport;
    for (uint8_t i = 0; i < open_ep_count; ++i) {
...
static bool pass_driver_control_xfer(uint8_t rhport, uint8_t stage,
    ...
    if (stage == CONTROL_STAGE_SETUP) {
        ...
        int64_t        previous_age_us = -1;
        uint16_t       active_seq;

        // Preserve the SETUP-path guard as a fast fallback, but make the
        // check/set atomic with the independent watchdog below. A missing
        // timestamp is treated as stale rather than creating a permanent
        // BUSY state.
        portENTER_CRITICAL(&ctl_lock);
        if (ctl_pending) {
            previous_seq = ctl_pending_seq;
            if (ctl_setup_us != 0) previous_age_us = now_us - ctl_setup_us;
            if (ctl_setup_us == 0 ||
                previous_age_us >= ((int64_t)CTL_PENDING_TIMEOUT_MS * 1000)) {
                ctl_pending      = false;
                ctl_setup_us     = 0;
                stale_cleared    = true;
            } else {
        ...
        active_seq = ctl_pending_seq;
        portEXIT_CRITICAL(&ctl_lock);

        if (stale_cleared) {
            char m[128];
            int n = snprintf(m, sizeof(m),
                "[L][CTL] STALE_CLEAR seq=%u age_ms=%lld source=setup pending=0\n",
                previous_seq, (long long)(previous_age_us / 1000));
            if (n > 0) km_uart_write(m, n);
        }
        if (busy) {
            char m[128];
            int n = snprintf(m, sizeof(m),
                "[L][CTL] BUSY seq=%u age_ms=%lld bmReq=%02x bReq=%02x wVal=%04x wIdx=%04x\n",
                previous_seq, (long long)(previous_age_us / 1000),
        ...
        char m[192];
        int n = snprintf(m, sizeof(m),
            "[L][CTL] PENDING_SET seq=%u pending=1 bmReq=%02x bReq=%02x wVal=%04x wIdx=%04x wLen=%u dir=%s\n",
            active_seq, request->bmRequestType, request->bRequest,
        ...
        if (!is_in && wLength > 0) {
            if (wLength > CTL_OUT_BUF_SZ) {
                portENTER_CRITICAL(&ctl_lock);
                if (ctl_pending && ctl_pending_seq == active_seq) {
                    ctl_pending  = false;
                    ctl_setup_us = 0;
                }
                portEXIT_CRITICAL(&ctl_lock);
                n = snprintf(m, sizeof(m),
                    "[L][CTL] OUT_TOO_LARGE seq=%u wLen=%u buf=%u pending=0\n",
                    active_seq,
        ...
        bool sent = ipc_send(FRAME_CTRL_SETUP, 0, active_seq, setup, 8);
        n = snprintf(m, sizeof(m),
                     "[L][CTL] SETUP_FWD seq=%u frame_len=8 sent=%u pending=%u\n",
                     active_seq, (unsigned)sent, (unsigned)sent);
        if (n > 0) km_uart_write(m, n);
        if (!sent) {
            portENTER_CRITICAL(&ctl_lock);
            if (ctl_pending && ctl_pending_seq == active_seq) {
    ...
    if (stage == CONTROL_STAGE_DATA) {
        bool pending;
        uint16_t active_seq;
        tusb_control_request_t pending_request;
        portENTER_CRITICAL(&ctl_lock);
        pending = ctl_pending;
        active_seq = ctl_pending_seq;
        pending_request = ctl_pending_request;
        portEXIT_CRITICAL(&ctl_lock);
        if (pending) {
            const bool is_in = (pending_request.bmRequestType & 0x80) != 0;
            const uint16_t dlen = pending_request.wLength;
            if (!is_in && dlen > 0 && dlen <= CTL_OUT_BUF_SZ) {
                uint8_t payload[8 + CTL_OUT_BUF_SZ];
                memcpy(payload, &pending_request, 8);
                memcpy(payload + 8, ctl_out_buf, dlen);
                bool sent = ipc_send(FRAME_CTRL_SETUP, 0, active_seq,
                                     payload, (uint16_t)(8 + dlen));

                char m[112];
                int n = snprintf(m, sizeof(m),
                    "[L][CTL] SETUP_FWD seq=%u frame_len=%u out_data_len=%u sent=%u pending=%u\n",
                    active_seq, (unsigned)(8 + dlen), (unsigned)dlen,
    ...
    if (stage == CONTROL_STAGE_ACK)  {
        ...
        char m[80];
        int n = snprintf(m, sizeof(m),
                         "[L][CTL] ACK seq=%u cleared=%u pending=0\n",
                         active_seq, (unsigned)was_pending);
        if (n > 0) km_uart_write(m, n);
        return true;
    }
    return false;
...
// Periodic recovery for a CTRL_STATUS frame lost on IPC. This runs outside
// the SETUP path so the state cannot remain wedged merely because Windows
// stopped issuing control requests after repeated EP0 stalls. A late status
// keeps its old sequence number and is ignored by pass_usb_control_status().
void pass_usb_control_watchdog(void) {
    const int64_t now_us = esp_timer_get_time();
    bool cleared = false;
    uint16_t stale_seq = 0;
    int64_t age_us = -1;

    portENTER_CRITICAL(&ctl_lock);
    if (ctl_pending) {
        stale_seq = ctl_pending_seq;
        if (ctl_setup_us != 0) age_us = now_us - ctl_setup_us;
        if (ctl_setup_us == 0 ||
            age_us >= ((int64_t)CTL_PENDING_TIMEOUT_MS * 1000)) {
            ctl_pending  = false;
            ctl_setup_us = 0;
            cleared      = true;
        }
    ...
    if (cleared) {
        char m[128];
        int n = snprintf(m, sizeof(m),
            "[L][CTL] STALE_TIMEOUT_CLEAR seq=%u age_ms=%lld timeout_ms=%u pending=0\n",
            stale_seq, (long long)(age_us / 1000),
            (unsigned)CTL_PENDING_TIMEOUT_MS);
        if (n > 0) km_uart_write(m, n);
    }
}
...
static bool pass_driver_xfer_cb(uint8_t rhport, uint8_t ep_addr,
    ...
    if (slot->is_in) {
        ...
        if (result == XFER_RESULT_SUCCESS && xferred) {
            ...
            char m[160];
            int ln = snprintf(m, sizeof(m), "[L][EP] OUT t=%lld ep=%02x len=%u hex=",
...
static bool submit_in_core(uint8_t ep_addr, const uint8_t *data, uint16_t len, bool is_synth) {
    ...
    // Real reports only: show the first 20 per EP, then one heartbeat every
    // 1000 reports. This proves EP_IN is still alive without flooding UART0.
    static uint32_t in_dbg_count[PASS_MAX_EPS] = {0};
    uint8_t slot_ix = (uint8_t)(slot - open_eps);
    if (!is_synth && slot_ix < PASS_MAX_EPS) {
        uint32_t report_count = ++in_dbg_count[slot_ix];
        bool emit = report_count <= 20 || (report_count % 1000) == 0;
        if (emit) {
            int64_t t_us = esp_timer_get_time();
            char m[160];
            int ln = snprintf(m, sizeof(m),
                              "[L][EP] IN t=%lld count=%lu ep=%02x len=%u hex=",
                              (long long)t_us, (unsigned long)report_count,
                              ep_addr, (unsigned)len);
...
//
// ctl_pending has its own 5 s watchdog, called periodically from main.c.
...
void pass_usb_disconnect(void) {
    pass_usb_stop_synth();
    bool cleared;
    uint16_t cleared_seq;
    portENTER_CRITICAL(&ctl_lock);
    cleared_seq  = ctl_pending_seq;
    cleared      = ctl_pending;
    ctl_pending  = false;
    ctl_setup_us = 0;
    portEXIT_CRITICAL(&ctl_lock);
    if (cleared) {
        char m[80];
        int n = snprintf(m, sizeof(m),
                         "[L][CTL] CLEAR seq=%u reason=disconnect pending=0\n",
                         cleared_seq);
        if (n > 0) km_uart_write(m, n);
    }
...
bool pass_usb_control_in_complete(uint16_t seq, const uint8_t *data, uint16_t len) {
    bool match;
    uint8_t rhport = 0;
    tusb_control_request_t pending_request = {0};
    portENTER_CRITICAL(&ctl_lock);
    ...
    char m[176];
    int n = snprintf(m, sizeof(m),
                     "[L][CTL] IN_DATA_RX seq=%u len=%u match=%u ignored=%u hex=",
                     seq, len, (unsigned)match, (unsigned)!match);
    ...
    return tud_control_xfer(rhport, &pending_request, (void *)data, len);
}
...
void pass_usb_control_status(uint16_t seq, uint8_t status) {
    ...
    portENTER_CRITICAL(&ctl_lock);
    expected_seq = ctl_pending_seq;
    match = ctl_pending && seq == ctl_pending_seq;
    if (match) {
        if (ctl_setup_us != 0) elapsed_us = now_us - ctl_setup_us;
        rhport          = ctl_rhport;
        ctl_pending     = false;
        ctl_setup_us    = 0;
    }
    pending_after = ctl_pending;
    portEXIT_CRITICAL(&ctl_lock);
    ...
    int n = snprintf(m, sizeof(m),
                     "[L][CTL] STATUS_RX seq=%u expected=%u st=%s(%u) dt_us=%lld match=%u ignored=%u pending=%u\n",
                     seq, expected_seq, stxt, status, (long long)elapsed_us,
                     (unsigned)match, (unsigned)!match, (unsigned)pending_after);
    if (n > 0) km_uart_write(m, n);
    if (!match) return;
    if (status != XFER_OK) {
        usbd_edpt_stall(rhport, 0x00);
    }
}
