...
// UART1: binary IPC to Right MCU (2 Mbps stability-test default).
...
static uint32_t rx_crc_fail_count = 0;
static int64_t  rx_last_good_us   = 0;
...
static void ipc_feed(uint8_t b) {
    ...
    case S_READ_CRC_HI: {
        ...
        if (crc == rx_crc_rcvd) {
            rx_last_good_us = esp_timer_get_time();
            ipc_handle_frame(rx_type, rx_ep, rx_seq, rx_buf, rx_len);
        } else {
            uint32_t count = ++rx_crc_fail_count;
            // A few early samples show onset; thereafter one line per 100
            // failures avoids making UART timing worse while still proving
            // whether the error count is growing.
            if (count <= 4 || (count % 100) == 0) {
                int64_t now_us = esp_timer_get_time();
                int64_t good_age_ms = rx_last_good_us
                                      ? (now_us - rx_last_good_us) / 1000
                                      : -1;
                ESP_LOGW(TAG,
                         "[L][IPC] CRC_FAIL count=%lu type=0x%02x len=%u seq=%u expected=0x%04x got=0x%04x last_good_age_ms=%lld",
...
void ipc_init(void) {
    ...
    const uart_config_t cfg = {
        .baud_rate           = (int)PASS_IPC_UART_BAUD,
    ...
    ESP_LOGI(TAG, "[L][IPC] UART%d up baud=%lu RX=%d TX=%d",
             IPC_UART_PORT, (unsigned long)PASS_IPC_UART_BAUD,
             IPC_UART_RX_PIN, IPC_UART_TX_PIN);
