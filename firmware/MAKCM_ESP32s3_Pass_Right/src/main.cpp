...
//   Serial1 : IPC UART → Left.  RX=GPIO2 (from Left's TX=GPIO2),
...
void ipc_handle_frame(uint8_t type, uint8_t ep_addr, uint16_t seq,
    ...
    case FRAME_EP_OUT:
        ...
        break;
    case FRAME_TARGET_MOUNTED:
        pass_host.set_target_mounted(true);
        break;
...
void setup() {
    Serial.begin(115200);
    IpcSerial.begin(PASS_IPC_UART_BAUD, SERIAL_8N1, IPC_UART_RX, IPC_UART_TX);
    ...
    int n = snprintf(boot_msg, sizeof(boot_msg),
                     "[IPC] Pass_Right boot baud=%lu\n",
                     (unsigned long)PASS_IPC_UART_BAUD);
    if (n > 0) {
