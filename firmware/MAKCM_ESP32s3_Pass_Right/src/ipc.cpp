...
//       dispatch via ipc_handle_frame(); CRC mismatches are counted and
//       reported sparingly to Left's UART0 log before resynchronizing.
...
static uint32_t rx_last_good_ms   = 0;
...
void ipc_feed(uint8_t b) {
    ...
    case S_READ_CRC_HI: {
        ...
        if (crc == rx_crc_rcvd) {
            rx_last_good_ms = millis();
            ipc_handle_frame(rx_type, rx_ep, rx_seq, rx_buf, rx_len);
        } else {
            uint32_t count = ++rx_crc_fail_count;
            if (count <= 4 || (count % 100) == 0) {
                uint32_t now_ms = millis();
                long good_age_ms = rx_last_good_ms
                                   ? (long)(now_ms - rx_last_good_ms)
                                   : -1L;
                char msg[160];
                int n = snprintf(msg, sizeof(msg),
                    "[IPC] CRC_FAIL count=%lu type=0x%02x len=%u seq=%u expected=0x%04x got=0x%04x last_good_age_ms=%ld",
                    (unsigned long)count, rx_type, (unsigned)rx_len,
                    (unsigned)rx_seq, (unsigned)crc,
                    (unsigned)rx_crc_rcvd, good_age_ms);
                if (n > 0 && n < (int)sizeof(msg)) {
                    ipc_send(FRAME_LOG, 0, 0,
...
// byte (especially when testing the original 5 Mbps setting).
