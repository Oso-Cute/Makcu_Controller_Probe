...
Walks the exact sequence:
  ...
  2. Disconnect all, hold the LEFT (USB1) button + plug in -> Flash Left.
  3. Disconnect Left, plug in the RIGHT (USB3) -> Flash Right.
  4. Disconnect, then reconnect: controller->RIGHT, PC->MIDDLE, Xbox->LEFT.
...
class FlashWizard:
    ...
    def render(self):
        ...
        if self.step == 0:
            ...
            self.instr.config(text=(
                ...
                "1) UNPLUG the middle cable and every other cable.\n"
                "2) HOLD the button next to USB1 (left).\n"
                "3) While holding, plug ONLY USB1 into this PC.\n"
                "4) Release. Wait for it to be detected below, then click "
                "‘Flash Left’."))
            ...
            self.instr.config(text=(
                ...
                "1) Disconnect all cables for about 10 seconds.\n"
                "2) Controller/XIM → RIGHT (USB3).\n"
                "3) PC → MIDDLE (USB2).\n"
                "4) Xbox → LEFT (USB1) last.\n\n"
    ...
    def check_comm(self, final=False):
        ...
        try:
            ...
            while time.monotonic() < deadline and b"kmbox:" not in raw:
                raw += s.read(s.in_waiting or 1)
            s.close()
            text = raw.decode("ascii", "replace").replace("\r", "")
            reply = next((line.strip() for line in text.split("\n")
                          if "kmbox:" in line), text.strip())
